...
main (int argc, char *argv[])
{
  ...
  */
  ...
  QuicHelper stack;
  stack.InstallQuic (nodes);//QuicL4ProtocolのtypeIDを持ったobjectをnode.getできるようにしている。
  ...
  QuicEchoServerHelper echoServer (9);//object factoryにechoserverのIDをセットして、引き数のポートをセットしている

  ApplicationContainer serverApps = echoServer.Install (nodes.Get (1));//QuicEchoServer型のオブジェクトをnodeにaddAplicationしている
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (1200.0));

  QuicEchoClientHelper echoClient (interfaces.GetAddress (1), 9);//ファクトリーにIDのセットと送信アドレス、ポートの設定
  echoClient.SetAttribute ("MaxPackets", UintegerValue (1));//ファクトリーに名前と値をセットしてる
  echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));//ファクトリーに名前と値をセットしてる
  //echoClient.SetAttribute ("MaxStreamData", UintegerValue (1024));

  ApplicationContainer clientApps = echoClient.Install (nodes.Get (0));
  echoClient.SetFill (clientApps.Get (0), "Hello World");//"hello worldってobjectを返却している"
 ...
 pointToPoint.EnablePcapAll("data/quic-tester/pcaps");
